...
n_time = 1716
n_lat = -1 # -1 for full data, when working with real world sst fields
n_lon = -4 # -4 for full data, when working with real world sst fields
...
missing_values = [0.9]#[0.99,0.95,0.9,0.75,0.5] # Set rates of missing values.
...
if source=='realworld':
    ...
    feature_anomaly = sst_anomaly_fields.values[:n_time,:n_lat,:n_lon]
    
    # Get mask for invalid grid points (=land masses):
    invalid_gridpoints = np.isnan(feature_anomaly)
    
    # Set invalid grid points to zero:
    feature_anomaly_nan_to_zero = np.copy(feature_anomaly)
    feature_anomaly_nan_to_zero[invalid_gridpoints] = 0
    
    # Flatten spatial dimensions.
    feature_anomaly_flat = feature_anomaly_nan_to_zero.reshape((n_time,-1))
    ...
    # Flatten spatial dimensions.
    feature_anomaly_flat = feature_anomaly_nan_to_zero.stack(tmp_space=("lat", "lon")).dropna(dim="tmp_space")
...
for i in range(len(missing_values)):
    ...
    for n_eof in np.arange(1,max_eofs+1):
        ...
        if (n_eof)%10 == 0:
            total_loss_filename = 'total_loss_all_'+str(n_eof)+'_eofs.npy'
            reserved_loss_filename = 'reserved_loss_all_'+str(n_eof)+'_eofs.npy'
            missing_loss_filename = 'missing_loss_all_'+str(n_eof)+'_eofs.npy'
            np.save(path_to_store_results / total_loss_filename, np.array(total_loss_all))
            np.save(path_to_store_results / reserved_loss_filename, np.array(reserved_loss_all))
            np.save(path_to_store_results / missing_loss_filename, np.array(missing_loss_all))
