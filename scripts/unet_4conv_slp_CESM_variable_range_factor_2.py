...
import sys
sys.path.append('GitHub/MarcoLandtHayen/reconstruct_missing_data/reconstruct_missing_data')
from data_loading import load_data_set, get_anomalies, clone_data, create_missing_mask, split_and_scale_data
...
augmentation_factor = 2 # Number of times, each sample is to be cloned, keeping the original order.
train_val_split = 0.8 # Set rel. amount of samples used for training.
missing_values = [0.75, 0.99] # Set array for desired amounts of missing values: 0.9 means, that 90% of the values are missing.
...
missing_max = missing_values[1]

# Create sub-directory to store results: Raise error, if path already exists, to avoid overwriting existing results.
os.makedirs(path / 'missing_' f'{int(missing_min*100)}' '_' f'{int(missing_max*100)}', exist_ok=False)
...
missing_mask = create_missing_mask(data=data, mask_type=mask_type, missing_type=missing_type, missing_min=missing_min, missing_max=missing_max, seed=seed)

# Store missing mask:
np.save(path / 'missing_' f'{int(missing_min*100)}' '_' f'{int(missing_max*100)}'/ 'missing_mask.npy', missing_mask)
...
model.save(path / 'missing_' f'{int(missing_min*100)}' '_' f'{int(missing_max*100)}'/ f'epoch_{0}')
...
for j in range(epochs):
    ...
    model.save(path / 'missing_' f'{int(missing_min*100)}' '_' f'{int(missing_max*100)}'/ f'epoch_{j+1}')
...
# Save loss:
np.save(path / 'missing_' f'{int(missing_min*100)}' '_' f'{int(missing_max*100)}'/ 'train_loss.npy', train_loss)
np.save(path / 'missing_' f'{int(missing_min*100)}' '_' f'{int(missing_max*100)}'/ 'val_loss.npy', val_loss)
