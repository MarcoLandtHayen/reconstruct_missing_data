...
# Work with surface air temperature (SAT) fields from Earth System Models, either FOCI or CESM.
#
...
if missing_values[1]==0.999:
    path = Path(
        ...
        + "_"
        + missing_type
        + "_" + f"{int(missing_values[0]*100)}"
        + "_" + f"{int(missing_values[1]*1000)}"
        + "_factor_"
...
if missing_values[1]==0.999:
    os.makedirs(
        path / "missing_" f"{int(missing_min*100)}" "_" f"{int(missing_max*1000)}",
        exist_ok=False,
    )
else:
    os.makedirs(
        path / "missing_" f"{int(missing_min*100)}" "_" f"{int(missing_max*100)}",
        exist_ok=False,
    )
...
if missing_values[1]==0.999:
    np.save(
        path / "missing_"
        f"{int(missing_min*100)}"
        "_"
        f"{int(missing_max*1000)}" / "missing_mask.npy",
        missing_mask,
    )
else:
    np.save(
        ...
        f"{int(missing_min*100)}"
        "_"
        f"{int(missing_max*100)}" / "missing_mask.npy",
        missing_mask,
    )
...
if missing_values[1]==0.999:
    model.save(
        ...
        f"{int(missing_min*100)}"
        "_"
        f"{int(missing_max*1000)}" / f"epoch_{0}"
    )
else:
    model.save(
        path / "missing_"
        f"{int(missing_min*100)}"
        "_"
        f"{int(missing_max*100)}" / f"epoch_{0}"
    )
...
for j in range(epochs):
    ...
    if missing_values[1]==0.999:
        model.save(
            ...
            f"{int(missing_min*100)}"
            "_"
            f"{int(missing_max*1000)}" / f"epoch_{j+1}"
        )
    else:
        model.save(
            path / "missing_"
            f"{int(missing_min*100)}"
            "_"
            f"{int(missing_max*100)}" / f"epoch_{j+1}"
        )
...
if missing_values[1]==0.999:
    np.save(
        ...
        f"{int(missing_min*100)}"
        "_"
        f"{int(missing_max*1000)}" / "train_loss.npy",
        train_loss,
    )
    np.save(
        path / "missing_"
        f"{int(missing_min*100)}"
        "_"
        f"{int(missing_max*1000)}" / "val_loss.npy",
        val_loss,
    )
else:
    np.save(
        ...
        f"{int(missing_min*100)}"
        "_"
        f"{int(missing_max*100)}" / "train_loss.npy",
        train_loss,
    )
    np.save(
        path / "missing_"
        f"{int(missing_min*100)}"
        "_"
        f"{int(missing_max*100)}" / "val_loss.npy",
        val_loss,
    )
