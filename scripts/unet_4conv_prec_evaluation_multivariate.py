...
# prec CESM:
#path_to_final_model='GitGeomar/marco-landt-hayen/reconstruct_missing_data_results/predict_unet_4conv_multivariate_prec_CESM_optimal_discrete_lead_0_final'
#path_to_final_model='GitGeomar/marco-landt-hayen/reconstruct_missing_data_results/predict_unet_4conv_multivariate_prec_CESM_optimal_discrete_lead_1_final'
path_to_final_model='GitGeomar/marco-landt-hayen/reconstruct_missing_data_results/predict_unet_4conv_multivariate_prec_CESM_optimal_discrete_lead_3_final'
...
prec_climatology_fields = data[target_feature].groupby("time.month").mean("time")

# Get prec anomaly fields by subtracting monthly climatology from raw slp fields:
prec_anomaly_fields = data[target_feature].groupby("time.month") - prec_climatology_fields
...
SPI_test_pred_all = np.zeros((len(missing_values),n_test))
SPI_test_target_all = np.zeros((len(missing_values),n_test))
...
for i in range(len(missing_values)):
    ...
    # Revert scaling: Use min / max for correct feature (prec).
    ...
    test_pred_xr_rescaled_fields = test_pred_xr_rescaled.groupby("time.month") + prec_climatology_fields
    test_target_xr_rescaled_fields = test_target_xr_rescaled.groupby("time.month") + prec_climatology_fields
        
    # Compute indices:
    SPI_test_pred = sahel_precipitation(test_pred_xr_rescaled_fields).values
    SPI_test_target = sahel_precipitation(test_target_xr_rescaled_fields).values
  
    # Store indices:
    SPI_test_pred_all[i] = SPI_test_pred
    SPI_test_target_all[i] = SPI_test_target
...
np.save(Path(path_to_final_model) / 'SPI_test_pred_all.npy', SPI_test_pred_all)
np.save(Path(path_to_final_model) / 'SPI_test_target_all.npy', SPI_test_target_all)
