...
def build_unet_4conv(
    input_shape, CNN_filters, CNN_kernel_size, learning_rate, loss_function
):
...
def build_unet_5conv(
    input_shape, CNN_filters, CNN_kernel_size, learning_rate, loss_function
):

    """Build and compile U-Net model with 5 convolutional layers.
    ...
    input_shape: list
        Contains triple: (latitude, longitude, 1), where trailing 1 denotes the number of channels.
    CNN_filters: np.ndarray of int
        Contains 5 numbers to specify the number of filters in each of the 5 convolutions.
    CNN_kernel_size: int
        Specifies the kernel size, taken to be similar for each convolution.
    learning_rate: float
        Learning rate.
    ...
    # Add max. pooling (2x2):
    pool_4 = MaxPool2D(pool_size=(2, 2))(cnn_8)
    ...
    # Add upsampling (2x2):
    up_1 = UpSampling2D(size=(2, 2))(cnn_10)

    # Add Concatenation:
    concat_1 = concatenate([up_1, cnn_8])
    ...
    # Add upsampling (2x2):
    up_2 = UpSampling2D(size=(2, 2))(cnn_12)

    # Add Concatenation:
    concat_2 = concatenate([up_2, cnn_6])
    ...
    # Add upsampling (2x2):
    up_3 = UpSampling2D(size=(2, 2))(cnn_14)

    # Add Concatenation:
    concat_3 = concatenate([up_3, cnn_4])
    ...
    # Add upsampling (2x2):
    up_4 = UpSampling2D(size=(2, 2))(cnn_16)

    # Add Concatenation:
    concat_4 = concatenate([up_4, cnn_2])
    ...
    # Add final convolution: Only ONE filter with filter size ONE!
    output = Conv2D(filters=1, kernel_size=1, padding="same", activation="relu")(cnn_18)
